...
import jiangxiaopeng.domain.conversation.application.command.GetMessageCommand;
import jiangxiaopeng.domain.conversation.application.command.SendMessageCommand;
import jiangxiaopeng.domain.conversation.application.command.SubmitFeedbackCommand;
import jiangxiaopeng.domain.conversation.application.dto.MessageDto;
import jiangxiaopeng.domain.conversation.application.dto.MessageListResponse;
import jiangxiaopeng.domain.conversation.application.service.MessageApplicationService;
import jiangxiaopeng.domain.conversation.application.service.StreamingChatService;
...
public class MessageController {
    ...
    public ApiResponse<MessageDto> sendMessage(
        ...
        var command = new SendMessageCommand(chatId, user.getUid(), request.content(), request.attachmentIds());
        return ApiResponse.ok(messageService.sendMessageSync(command));
    }

    @Operation(summary = "发送消息（流式 SSE）", description = "返回 text/event-stream，增量推送助手回复（event: ai_token）。")
    ...
    public Flux<ServerSentEvent<String>> regenerateReply(
            @AuthenticationPrincipal UserPrincipal user,
            @Parameter(description = "会话 ID") @PathVariable String chatId,
            @Parameter(description = "消息 ID") @PathVariable String msgId) {
        var command = new SendMessageCommand(chatId, user.getUid(), "", List.of());
        return streamingChatService.execute(command);
    }
